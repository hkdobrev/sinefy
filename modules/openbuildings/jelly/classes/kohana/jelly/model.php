 ...
 */
abstract class Kohana_Jelly_Model extends Model {
	...
	public function __construct($key = NULL, $meta_name = NULL)
	{
		...
		$this->_meta = Jelly::meta($meta_name);
		...
		if ($key)
		{
			$result = Jelly::query($meta_name, $key)
				->as_object(FALSE)
				->select();
	...
	{
		if ( ! $this->check())
			throw new Jelly_Validation_Exception($this->_meta->errors_filename(), $this->_validation);
	 ...
	 * Automatically loads the error messages file from messages/jelly-validation/<model_name>
	...
	public function save($validate = NULL)
	{
		...
		foreach ($this->_retrieved as $column => $value) 
		{
			if ( ! isset($this->_changed[$column])
				...
				AND $association instanceof Jelly_Association_Collection 
				AND $value->changed())
			{
	...
	{
		if ( ! $this->loaded())
			throw new Kohana_Jelly_Exception_NotLoaded("Model not loaded", $this);
