 ...
 */
abstract class Kohana_Jelly_Builder extends Database_Query_Builder_Select {
	...
	public function __call($method, $args)
	{
		...
		throw new Jelly_Exception_MethodMissing($this, $method, $args);
	}
	 ...
	 * If the passed array is empty - an empty Jelly_Collection will be returned
	 * 
	...
	public function find($ids = NULL)
	{
		...
		elseif (empty($ids))
		{
			return new Jelly_Collection(array(), Jelly::class_name($this->_model));
		}
	...
	/**
	 * If an image is not loaded, rise a Jelly_Exception_NotLoaded exception. 
	 * If you pass an array of ids it will throw an exception if any of the ids is missing.
	 * 
	 * @throws Jelly_Exception_NotLoaded If item is not found in the database
	 * @param  integer|array $ids 
	 * @return Jelly_Model|Jelly_Collection
	 */
	public function find_insist($ids = NULL)
	{
		...
		if (is_array($ids))
		{
			...
			if ( ! empty($missing_ids))
				throw new Jelly_Exception_NotFound("Not found model with ids :model", join(', ', $missing_ids));	
		}
		elseif ( ! $result->loaded())
		{
			throw new Jelly_Exception_NotFound("Not found model :model", $result);
		}
	...
	public function select_all($db = NULL)
	{
		...
		if ($meta)
		{
			...
			Jelly::global_trigger('builder.before_select', $this);
		}
		...
		$this->_result = new Jelly_Collection($this->_result, $this->_as_object);
		...
		if ($meta)
		{
			...
			Jelly::global_trigger('builder.after_select', $this);
		}
	...
	public function insert($db = NULL)
	{
		...
		Jelly::global_trigger('builder.before_insert', $this);
		...
		Jelly::global_trigger('builder.after_insert', $this);
	...
	public function update($db = NULL)
	{
		...
		Jelly::global_trigger('builder.before_update', $this);
		...
		Jelly::global_trigger('builder.after_update', $this);
	...
	public function delete($db = NULL)
	{
		...
		if ($meta)
		{
			...
			Jelly::global_trigger('builder.before_delete', $this);
		}
		...
		if ($meta)
		{
			...
			Jelly::global_trigger('builder.after_delete', $this);
	...
	public function join_association($association, $type = NULL)
	{
		...
		foreach ($associations as $name => $name_alias) 
		{
			...
			$meta = Jelly::meta($association->foreign());
		}
		return $this;
	}
	...
	public function with($relationship)
	{
		...
		foreach ($paths as $path)
		{
			...
			$association = Jelly::meta($parent_model)->association_insist($path);
		...
		if ( ! isset($this->_model_cache[$original]))
		{
			...
			elseif (($pos = strpos($model, ':')) !== FALSE)
			{
				...
				while ( ! empty($chain))
				{
					...
					$parent = Jelly::meta($parent)->association($association)->foreign['model'];
				}
	...
	/**
	 * You can inspect some of the parameters of the jelly builder - very useful for extensions.
