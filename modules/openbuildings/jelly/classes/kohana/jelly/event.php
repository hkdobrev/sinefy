 ...
 */
abstract class Kohana_Jelly_Event {
	 ...
	 * @param  mixed  $sender Jelly_Model, Jelly_Builder or Jelly_Meta
	 * @param  string $method 
	 * @param  array  $params passed to the method
	 * @return mixed          returns the response from the callback
	 * @throws Jelly_Exception_MethodMissing If no method is found
	 */
	public function trigger_callback($type, $sender, $method, $params)
	{
		...
		if (empty($this->_events[$event]))
			throw new Jelly_Exception_MethodMissing($sender, $method, $params);
