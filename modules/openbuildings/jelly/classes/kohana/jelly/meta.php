 ...
 */
abstract class Kohana_Jelly_Meta {
	...
	public function finalize($model)
	{
		...
		Jelly::global_trigger('meta.before_finalize', $this);
		...
		if ( ! $this->_errors_filename)
		{
			...
			$this->_errors_filename = 'jelly-validation/'.$this->_model;
		}
		...
		foreach ($this->_fields as $column => $field)
		{
			...
			if ($field instanceof Jelly_Field AND $field->primary AND empty($this->_primary_key))
			{
		...
		foreach ($this->_associations as $column => & $association)
		{
			...
			{
				$association = Jelly::association($association);
			}
		...
		Jelly::global_trigger('meta.after_finalize', $this);
	}
	 ...
	 * @return  Jelly_Association|Jelly_Meta|null
	 */
	 ...
	 * @return  array|Kohana_Jelly_Meta
	 */
