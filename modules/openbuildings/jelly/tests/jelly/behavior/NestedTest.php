 ...
 * @group   jelly.behavior.nested
 */
class Jelly_Behavior_NestedTest extends Unittest_Jelly_TestCase {

	
	public function test_parent()
	{
		$three = Jelly::factory('test_category', 3);
	...
	{
		$one = Jelly::factory('test_category', 1);

		$this->assertEquals(array(3), $one->children->ids());

		$top = Jelly::query('test_category')->root()->select_all()->ids();
		$this->assertEquals(array(1, 2, 4), $top);
	}

	public function test_children()
	{
		$cat = Jelly::query('test_category')->find_insist(array(1, 2, 3, 4))->as_array();

		$cat[1]->children = array($cat[2], $cat[3]);
		$cat[0]->children = array($cat[1]);

		$cat[] = Jelly::query('test_category')->find(array(1, 2, 3, 4));
		$cat[0]->save();
	...
	{
		$author = Jelly::factory('test_author');
