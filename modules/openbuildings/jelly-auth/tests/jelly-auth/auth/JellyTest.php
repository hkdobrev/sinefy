 ...
 * @group   jelly
 * @group   jelly.auth
 * @group   jelly.auth.jelly
 */
class Auth_JellyTest extends Unittest_Auth_TestCase {
	...
	{
		$user = Jelly::factory('test_user', 1);
	...
	public function test_login($login_args, $should_be_logged_in)
	{
		...
		if ($login_args[0])
		{
			$login_args[0] = Jelly::factory('test_user', 1)->get($login_args[0]);
		}
		else
		{
			$login_args[0] = $user;
	...
	public function test_remember()
	{
		...
		$jelly_token = Jelly::factory('test_user_token')->get_token($token->token);
		
		$this->assertTrue($jelly_token->loaded());
	...
	{
		$token = Jelly::factory('test_user_token', $token_id);
	...
	public function test_login_with_service()
	{
		...
		$facebook
			->expects($this->at(1))
			->method('login')
			->will($this->returnValue(Jelly::factory('test_user', 1)));
