...
class Auth_Model_UserTest extends Unittest_Auth_TestCase {
	...
	{
		$builder = Jelly::query('test_user', $key);
	...
	{
		Jelly::clear_cache('test_user');
		Jelly::meta('test_user')->add_password_validation();
		$user = Jelly::factory('test_user', 1)->set($params);
	...
	public function test_generate_login_token()
	{
		...
		$retrieved_token = Jelly::factory('test_user_token')->get_token($token->token);
