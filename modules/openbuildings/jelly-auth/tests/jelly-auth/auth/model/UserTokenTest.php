 ...
 * @group   jelly
 * @group   jelly.auth
 * @group   jelly.auth.model
 * @group   jelly.auth.model.user_token
 */
class Auth_Model_UserTokenTest extends Unittest_Auth_TestCase {
	...
	{
		$token = Jelly::factory('test_user_token', $id);
	...
	{
		$token = Jelly::factory('test_user_token')->get_token($token);
		$this->assertEquals($is_loaded, $token->loaded());
	}

	public function test_delete_expired()
	{
		$this->assertEquals(3, Jelly::query('test_user_token')->count());

		Jelly::factory('test_user_token')->delete_expired();

		$this->assertEquals(2, Jelly::query('test_user_token')->count());
	}

	public function test_create_token()
	{
		$token = Jelly::factory('test_user_token')->create_token(array('test_user_id' => 2, 'user_agent' => 'new agent'));
